...
import { IconFilter, IconMessage, IconUser } from "@tabler/icons-react"
...
const messageLogData = [
  ...
  },
  {
    ...
    user: "Kim Min-ji",
    type: "ai",
    message: "Great! Would you like to review your progress on last week's goals?",
  ...
  },
  {
    ...
    user: "Park Ji-sung",
    type: "coach",
    message: "Hi Park Ji-sung, I'm following up on our last session. How is your progress with the stress reduction techniques we discussed?",
  ...
  },
  {
    ...
    user: "Park Ji-sung",
    type: "user",
    message: "I've been practicing the breathing exercises daily and they're helping a lot!",
          ...
          <ScrollArea className="h-[400px] pr-4">
            <div className="space-y-4">
              {filteredMessages.map((message) => (
                <div 
                  key={message.id} 
                  className={`flex gap-3 p-3 rounded-lg ${message.type === 'user' ? 'bg-muted/40' : 'bg-muted/20'}`}
                >
                  <div className={`flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-full ${
                    message.type === 'ai' ? 'bg-primary/10 text-primary' : 
                    message.type === 'coach' ? 'bg-secondary/10 text-secondary' :
                    'bg-muted'
                  }`}>
                    {message.type === 'ai' ? <IconMessage size={16} /> : 
                     message.type === 'coach' ? <IconUser size={16} /> :
                     <IconUser size={16} />}
                  </div>
                    ...
                    <div className="flex justify-between items-center mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {message.type === 'ai' ? '(AI Chatbot)' : 
                           message.type === 'coach' ? '(Coach Message)' : 
                           '(User)'}
                        </span>
                      </div>
                      <span className="text-xs text-muted-foreground">
