...
import {
  ...
  useSessions,
} from "@/services/sessions/session.hooks";
import {
  generateSessions,
...
export function SessionCreationModal({
  ...
  const isEdit = sessions?.data?.results?.length || 0 > 0;
  ...
  const handleSubmit = async (data: CreateMultipleSessionsSchemaType) => {
    ...
    try {
      ...
      const result = await createMultipleSessionsMutation.mutateAsync(
        formattedData
      );
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center text-xl">
            ...
            {isEdit ? "Update Sessions" : "Create Sessions"} for{" "}
            {selectedProgram.name}
          </DialogTitle>
          <DialogDescription>
            {isEdit
              ? "Update multiple sessions for this program. Use the template to quickly generate sessions or add them individually."
                ...
                >
                  ...
                  {creationResults && (
                    <Card className="mt-4">
                      ...
                      <CardContent>
                        <div className="space-y-2">
                          {creationResults.results.map(
                            (result: any, index: number) => (
                              <div
                                key={`result-${result.sessionData.name}-${index}`}
                        ...
                        {creationResults.results.some(
                          (r: any) => !r.success
                        ) && (
                          <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded">
                            <p className="text-sm text-yellow-800">
                              Some sessions failed to create. You can try
                              creating them again or modify the details.
                            </p>
                          </div>
                        )}
                      </CardContent>
                  ...
                  <div className="flex justify-end space-x-2 pt-4">
                    ...
                    >
                      ...
                      {isEdit ? "Update Sessions" : "Create Sessions"}
                      {fields.length !== 1 ? "s" : ""}
                    </Button>
                  </div>
                </form>
